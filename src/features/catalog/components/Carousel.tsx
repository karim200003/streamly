...
export default function Carousel({ title, items, priority, fancy }: CarouselProps) {
  ...
  // works natively.
  //
  // Registered natively with { passive: false } rather than via React's
  // onWheel prop: React attaches `wheel` at the root as a passive
  // listener, so `preventDefault()` inside a React handler is silently
  // ignored and the page scrolls vertically at the same time.
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
